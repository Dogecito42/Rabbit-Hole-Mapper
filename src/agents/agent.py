...
def _normalizar_categoria(categoria) -> str:
    """
    Estandariza el nombre de categoria devuelto por el LLM.

    El LLM produce variantes inconsistentes del mismo concepto
    ("humorcostumbrista", "humor costumbrista", "Humor Costumbrista") que
    fragmentan el espacio de estados del Q-Learning. Normalizar aqui garantiza
    que todos los caminos de datos (Decision, JSON, Q-table) usan la misma clave.
    """
    if not categoria:
        return "desconocida"
    return re.sub(r"\s+", " ", categoria.strip().lower())
...
class Agent:
    ...
    def evaluar(self, video: VideoMetadata) -> Decision:
        ...
        categoria = _normalizar_categoria(datos["categoria_detectada"])
