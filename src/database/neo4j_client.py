        ...
        with self._driver.session() as session:
            session.run(
                ...
                SET v.titulo   = COALESCE($titulo, v.titulo),
                    v.hashtags = CASE WHEN size($hashtags) > 0 THEN $hashtags
                                      ELSE COALESCE(v.hashtags, []) END
                """,
                video_id=video_id,
                titulo=titulo,
                hashtags=hashtags,
